...
public:
    ...
    explicit Logger(int value_ = 0) : m_id(value_) {
        ++instance_count;
    }

    Logger(const Logger& rhs) : m_id(rhs.m_id) {
        ++instance_count;
    }

    Logger(Logger&& rhs) noexcept : m_id(rhs.m_id) {
        ++instance_count;
    }
...
void TestMethodInvocation() {
    ...
    base_closure.clear();
    res = child_inst.Call("test"s, {ObjectHolder::Own(String{"value1"s}), ObjectHolder::Own(String{"value2"s})}, context);
